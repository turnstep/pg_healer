...
pg_healer_checksum(char *page, BlockNumber blockNumber)
{
	...
	/*
	 * The checksum itself must be zeroed out before computing a checksum on
	 * the page, so we squirrel the old value away
	 */
...
pg_healer_error_hook(ErrorData * edata)
{
	...
	/*
	 * Continue chain to previous hook (which may well report on an already
	 * fixed error!)
	 */
	if (prev_log_hook)
		(*prev_log_hook) (edata);
...
pg_healer_corrupt(PG_FUNCTION_ARGS)
{
	...
	/*
	 * "freespace" Put non-zero bytes at start and end of freespace
	 */
	...
	/*
	 * "checksum" Make the checksum 00, which should never be valid
	 */
	else if (strcmp(action, "checksum") == 0)
	{
		*(pageBuffer + 8) = 0;
		*(pageBuffer + 9) = 0;
		ereport(INFO, (errmsg("Checksum forced to zero"),
					   errtable(rel)
					   ));
	}

	/*
	 * "pd_lsn" Mess up the pd_lsn in the header
	 */
	...
	/*
	 * "pd_special" Mess up the pd_special in the header
	 */
	...
	/*
	 * "pd_pagesize_version" Mess up the pd_pagesize_version in the header
	 */
...
pg_healer_repair_buffer(char *pageBuffer, BlockNumber blockNumber, uint16 relType)
{
	...
	int			header_size,
				header_version;
	int			good_version = 4;		/* TODO: Adjust to < 4 for really old
...
pg_healer_repair_external(BlockNumber blockNumber, char *pageBuffer, char *filePath)
{
	...
	/*
	 * If the checksums match, we assume we can simply copy the whole page
	 * over
	 */
	...
	/*
	 * In case a cosmic ray hit the checksum itself
	 */
	if (actual_checksum == ext_header_checksum)
	{
		pageBuffer[8] = ext_pageBuffer[8];
		pageBuffer[9] = ext_pageBuffer[9];
		return 2;
	}


	/*
	 * Otherwise, we have to be a little more deliberate. We want to try the
	 * most obvious solutions first, and keep checking the checksum along the
	 * way.
	 */
	...
	/*
	 * First, copy over row by row, to see if the corruption is inside the
	 * rows. We only copy rows that exist on both sides, obviously. There are
	 * much more brains to be added here, but this is a pretty good start: it
	 * should catch many single-bit errors, assuming the external source is
	 * fairly up to date.
	 */
	row_diffs = 0;
	for (i = 24; i < ext_pd_lower; i += 4)
	{
		...
		/*
		 * elog(INFO, "Looking for pointer at byte %d, pd_lower is %d", i,
		 * ext_pd_lower);
		 */
...
pg_healer_repair_path(char *filePath, BlockNumber blockNumber)
{
	...
	/*
	 * Grab what this page thinks the checksum should be, and what it actually
	 * is
	 */
	memcpy(&header_checksum, &pageBuffer[8], 2);
	actual_checksum = pg_healer_checksum(pageBuffer, blockNumber);
